		...
		<table class="table-group">
			<tr>
				...
				<td>
					<div class="ranges-group"><div class="range-10">
						<input type="text" name="user_id" value="<c:out value="${param.user_id}" />" class="form-input" maxlength="30" data-valid="<spring:message code="FLD.COM.0148"/>,required,alphanum">
					</div></div>
				</td>
				<td rowspan="5" style="width:148px; border: 1px solid #d8d8d8;" title="사진추가">
					<div style="width: 100%; height: 100%; cursor: pointer;" id="fileUpload"><div class="user-imgupload-text">사진을 등록해주세요</div></div>
					<input type="file" name="file_upload" style="display:none;" accept=".jpg,.jpeg,.png">
				</td>
			</tr>
			<tr>
			...
			<c:forEach var="map" items="${comUserRoleList}" >
				<c:if test="${multipleManagerRoleYn == 'Y'}"> 
						<option value="${map.role_id}"><c:out value="${map.role_nm}" /></option>
				</c:if>
			</c:forEach>
					</select>
					</div>
			...
			<tr>
				...
				<td colspan="2"><input type="text" name="description" class="form-input" maxlength="300"></td>
			</tr>
		</table>
